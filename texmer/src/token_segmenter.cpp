...
namespace texmer {
  ...
  DocumentSegments TokenSegmenter::segment(const Document& tokens,
    ...
    size_t current_id = segment_ids.at(0);
    size_t previous_id = current_id;
    String current_segment{tokens.at(0)};
  ...
  IntVector TokenSegmenter::get_segment_ids(const IntVector& boundaries,
    ...
    size_t n_segs = boundaries.size() + 1;
    size_t id_idx = 0;
  ...
  void TokenSegmenter::fill_with_segment_ids(IntVector& segment_ids, size_t& from,
                                             int with, unsigned times) const {
    for (unsigned i = 0; i < times; ++i)
      segment_ids.at(from++) = with;
  }

  void TokenSegmenter::check_boundaries(const IntVector& boundaries, size_t limit) const {
